...
| [grounding.md](grounding.md) | **derived, and three of the four derivations lose to their own control** - can a learning algorithm be derived from the generator rather than borrowed. The box ladder `1:4:9` and the spring's `1:2:3` are **one formula at two densities** - Rayleigh-Ritz on a Fokker-Planck Dirichlet form - recovered to five decimals with no transition data, which makes `quantising.md`'s dynamical ladder a prediction the histogram can contradict. On Range Break 100 the density reads **1.911** against a simulated spring's 1.913 and separates box from spring at a single-cut error of **0.003** where the transition matrix manages 0.025 - but a Kramers-Moyal generator manages **0.000**, so the classical control wins that question. What is new is **`R`**, the ratio of the two estimates, which is 1.00 on a diffusion, **2.0 on independent data** and 3.18 when the marginal is a scale mixture the dynamics do not share. Pointed at `barriers.py`: the closed form is **best of five arms at all ten geometries** on feeds `deriving.md` proves Brownian, wrong by **55.7 standard errors** on Boom and Crash, unbiased on real feeds at symmetric geometries and understating the far barrier by **+0.83 points [0.52, 1.13]** at 3:1, mirror-antisymmetrically, 17 of 19 feeds - and the feed's own return histogram removes **96%** of it while a learned volatility operator removes none. The largest number is that **`barriers.duration` is short by 25% at 3:3 and 96% at 10:10 on 19 of 19 real feeds**, against 5-9% on the synthetics, because the volatility half-life is **56 bars** where the synthetics' is 5. And the ceiling: a real feed's past and future share **one** degree of freedom (`rho_1` 0.64-0.83, `rho_2` 0.036-0.080, controls on their shuffled floor), so TICA loses to PCA at every rank and a **flat 20-bar mean captures 95.6%** of what eight constructions reach |
