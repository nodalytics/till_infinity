...
| [quantising.md](quantising.md) | **derived, one measurement, and a control that killed the headline test** - the Wick rotation makes each generator a textbook quantum system, so the propagators and boundary conditions are already statements about these prices. The **0.5826** is the *extrapolation length* of a discretely monitored wall - a Robin condition, one image charge moved - recovered at **0.58226 ± 0.00058** and **flat in barrier distance from 2 to 12 sigmas**, which is what makes it a wall rather than a fit, with the moved plane 50-70x better than the textbook one and a wrong-constant control only a third better. Two further internal routes agree to five figures, and the same propagation returns **0.3073** for deriving.md's 3:1 barrier against the 0.307 it measured on 174,501 trades. **The Range Break ladder does not work and the control is the finding**: a driftless random walk returns `lambda_2/lambda_1 = 3.55 ± 1.02` against a true box's 3.99, so **1:4:9 is evidence of a finite window, not of confinement** - binning any diffusion by its own quantiles makes the window the box. What survives is the knee in `lambda_1` (10 sigma) and the stationary kurtosis, separating box from spring from free at **0.0% single-cut error**; WKB then makes 1:4:9 a *ceiling*, so rebuilding.md's soft edge must land the ratio between 2 and 4, and both pages independently put its twenty-minute residual at **1.6 relaxation times of a 60-step box**. The one real measurement is the reconstruction, where the classical control is an **identity** - Kalman, Brownian bridge and linear interpolation coincide to `7.1e-15` for a walk seen at closes - so all the content is what the high and low add: **+13.1% btc, +13.0% gold, +13.3% eurusd** over linear on real 15m bars rebuilt at 1m, where the OHLC zigzag is *worse* than linear on all four |
