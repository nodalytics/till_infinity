...
| [field liveness](2026-09-11-field-liveness-design.md) | design | `run_vol` and `pivot` are identically zero across 20,000 production outcomes, and a research direction was nearly built on an option field that is dead at the source. Nothing systematically asks whether a column carries anything |
| [conditioning by default](2026-09-11-conditioning-by-default-design.md) | design | Simpson's paradox has reversed a pooled figure here **four times**. The rule is correct and does not survive contact with a new question, so it becomes mechanical: cut within strata by default, and shout when the ordering flips |
| [VIX as an ensemble member](2026-09-11-vix-ensemble-member-design.md) | design | The only positive result in this folder, unwired. Joins the scored volatility ensemble for four feeds at 1d and 1w, seeded from history because the warm-up is three months |
