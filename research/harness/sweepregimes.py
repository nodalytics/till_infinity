...
def walk(rows, start, trade, cost):
    """R under `sweep-aware`'s own exit, or None if there is no window."""
    ...
    for i in range(start, min(start + HOLD, len(rows))):
        _ts, _open, high, low, _close = rows[i]
        ...
        if hit_stop:  # the worse order within the bar, as `exits.py` assumes
    ...
    for feed, part in sorted(by_feed.items(), key=lambda kv: -len(kv[1])):
        ...
        for t in part:
            ...
            if start >= len(rows) - 10:
                continue
            r = walk(rows, start, t, t["level"] * bps / 10_000.0)
            if r is not None:
                t["r"] = r
                done.append(t)
    conn.close()
