...
# **The walk lives in the package now**, where CI lints it and the suite covers
# it. Two copies of it diverged here and one was wrong; see
# `research/specs/2026-09-11-replay-kernel-design.md` for why a forward walk is
# library code even though only research calls it.
...
def replay(trades, bars_db, spread):
    ...
    for feed, part in sorted(by_feed.items(), key=lambda kv: -len(kv[1])):
        ...
        for t in part:
            ...
            got = walk(rows, start, t, cost=cost, hold=HOLD, policy=EXIT_POLICY)
